...
void *threaded_merge_sort(void *arg) {
    ...
    if (left < right) {
        ...
        if (current_threads >= max_threads) {
            ...
            threaded_merge_sort(&(ThreadData){array, left, mid, max_threads});
            threaded_merge_sort(&(ThreadData){array, mid + 1, right, max_threads});
        } else {
            current_threads++;
            pthread_mutex_unlock(&thread_count_mutex);

            ThreadData left_data = {array, left, mid, max_threads};
            ThreadData right_data = {array, mid + 1, right, max_threads};
...
void parallel_merge_sort(int *array, int size, int max_threads) {
    ...
    ThreadData main_data = {array, 0, size - 1, max_threads};
